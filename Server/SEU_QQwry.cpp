...
{
    if (ioffset > m_be.uEOff) return TEXT("");
    char ch[4] = {0};
    CString buf;
...
void SEU_QQwry::SaveToFile(CString Name)
{
    ...
    for (m_i = 0; m_i < GetRecordCount(); m_i++)
    {
        ...
        str1.Format(TEXT("%d.%d.%d.%d"), m_ipoff.b3, m_ipoff.b2, m_ipoff.b1, m_ipoff.b0);//开始IP
        str2.Format(TEXT("%d.%d.%d.%d"), m_ei.b3, m_ei.b2, m_ei.b1, m_ei.b0);//结束IP
        m_buf.Format(TEXT("%-15s %-15s %s\r\n"),
...
{
    if (szIP.IsEmpty())return TEXT("请输入IP地址");
